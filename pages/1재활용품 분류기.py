...
with tab1:
    ...
    if uploaded_file is not None:
        ...
        for i in range(co):
            if clss[i] == 0:
                col2.write(f''':heavy_check_mark: :blue-background[종이팩] (약 {np.round(conff[i],2)*100} 퍼센트의 확률)  
        ...
        col2.write('')
        col2.write('')
        col2.write(':bulb: 2024년 8월 환경부 기준으로 분류한 것이며, 추후 변경될 수 있습니다.')
        col2.write(':bulb: 인식 결과에 오류가 있을 수 있습니다.')
...
with tab2:
    ...
    if picture:
        ...
        for i in range(co):
            if clss[i] == 0:
                st.write(f''':heavy_check_mark: :blue-background[종이팩] (약 {np.round(conff[i], 2) * 100} 퍼센트의 확률)  
                            플라스틱 빨대 등이나 이물질이 없는지 다시 한번 확인하고 말려서 배출해 주세요.  
                            강서구,성동구,금천구,강동구와 같이 주민센터 등에 전용수거함이 없는 경우 종이류와 섞이지 않게 묶어서 배출해 주세요.  
                            노원구의 경우 종이컵과 통합배출합니다.   
                            종이팩 분리배출과 관련하여 자세한 사항은 분리배츨 가이드 페이지를 참고해주세요!''')
            ...
            if clss[i] == 1:
                st.write(f''':heavy_check_mark: :blue-background[종이컵] (약 {np.round(conff[i], 2) * 100} 퍼센트의 확률)  
                            이물질이 없는지 다시 한번 확인 후 종이컵으로 배출해 주세요.  
                            종이류, 종이팩과 분리하여 배출합니다. 단, 노원구의 경우 종이팩과 통합배출합니다.  
                            종이컵 분리배출과 관련하여 자세한 사항은 분리배츨 가이드 페이지를 참고해주세요!''')

            if clss[i] == 2:
                st.write(f''':heavy_check_mark: :blue-background[종이컵+이물질] (약 {np.round(conff[i], 2) * 100} 퍼센트의 확률)  
                            이물질이 묻어있는 종이컵으로 보입니다. 이물질 없이 깨끗이 씻은 후 말린 상태로 배출해 주세요.  
                            만약 이물질 제거가 힘들다면, 일반쓰레기로 배출해 주세요.  
                            종이컵 분리배출과 관련하여 자세한 사항은 분리배츨 가이드 페이지를 참고해주세요!''')

            if clss[i] == 3:
                st.write(f''':heavy_check_mark: :blue-background[플라스틱] (약 {np.round(conff[i], 2) * 100} 퍼센트의 확률)  
                            혼합플라스틱 또는 타재료와 함께 배출할 수 없으며, 이물질이 없는지 다시 한 번 확인 후 배출해 주세요.  
                            노원구의 경우 기판을 분리한 전화기 또는 금속부분을 분리한 장난감 등은 배출할 수 있습니다.  
                            관악구와 서대문구는 노끈을 플라스틱/비닐 등 원재료에 따라 분리배출 가능합니다.  
                            플라스틱 분리배출과 관련하여 자세한 사항은 분리배츨 가이드 페이지를 참고해주세요!''')
            ...
            if clss[i] == 4:
                st.write(f''':heavy_check_mark: :blue-background[플라스틱+이물질] (약 {np.round(conff[i], 2) * 100} 퍼센트의 확률)  
                            이물질이 묻어있는 플라스틱으로 보입니다. 이물질 없이 깨끗이 씻은 후 배출해 주세요.  
                            만약 이물질 제거가 힘들다면, 일반쓰레기로 배출해 주세요.  
                            관악구와 서대문구는 노끈을 플라스틱/비닐 등 원재료에 따라 분리배출 가능합니다.   
                            플라스틱 분리배출과 관련하여 자세한 사항은 분리배츨 가이드 페이지를 참고해주세요!''')
            ...
            if clss[i] == 5:
                st.write(f''':heavy_check_mark: :blue-background[페트] (약 {np.round(conff[i], 2) * 100} 퍼센트의 확률)  
                            이물질 또는 다른 재질은 없는지 확인 후 배출해 주세요.  
                            가능한 색상별로 분리배출해 주시되, 유색페트병의 경우 종로구는 배출불가, 동대문구는 플라스틱류로 배출해 주세요.   
                            은평구에서는 불투명한 페트병도 배출 불가능합니다.  
                            페트 분리배출과 관련하여 자세한 사항은 분리배출 가이드 페이지를 참고해주세요!''')
            ...
            if clss[i] == 6:
                st.write(f''':heavy_check_mark: :blue-background[페트+이물질] (약 {np.round(conff[i], 2) * 100} 퍼센트의 확률)  
                            이물질이 묻어있는 페트로 보입니다. 이물질 없이 깨끗이 씻은 후 배출해 주세요.  
                            만약 이물질 제거가 힘들다면, 일반쓰레기로 배출해 주세요.  
                            가능한 색상별로 분리배출해 주시되, 유색페트병의 경우 종로구는 배출불가, 동대문구는 플라스틱류로 배출해 주세요.   
                            은평구에서는 불투명한 페트병도 배출 불가능합니다.  
            ...
            if clss[i] == 7:
                st.write(f''':heavy_check_mark: :blue-background[페트+다중포장재] (약 {np.round(conff[i], 2) * 100} 퍼센트의 확률)  
                            다중포장재가 포함된 페트로 보입니다. 포장재를 제거한 후 배출해 주세요.  
                            만약 포장재 제거가 힘들다면, 일반쓰레기로 배출해 주세요.  
                            가능한 색상별로 분리배출해 주시되, 유색페트병의 경우 종로구는 배출불가, 동대문구는 플라스틱류로 배출해 주세요.   
                            은평구에서는 불투명한 페트병도 배출 불가능합니다.  
            ...
            if clss[i] == 8:
                st.write(f''':heavy_check_mark: :blue-background[페트+이물질+다중포장재] (약 {np.round(conff[i], 2) * 100} 퍼센트의 확률)  
                            이물질과 다중포장재가 포함된 페트로 보입니다. 이물질과 포장재를 제거한 후 배출해 주세요.  
                            만약 제거가 힘들다면, 일반쓰레기로 배출해 주세요.  
                            가능한 색상별로 분리배출해 주시되, 유색페트병의 경우 종로구는 배출불가, 동대문구는 플라스틱류로 배출해 주세요.   
                            은평구에서는 불투명한 페트병도 배출 불가능합니다.  
        ...
        st.write(':bulb: 인식 결과에 오류가 있을 수 있습니다.')
